...
export const testimonialService = {
  ...
  async create(data: z.infer<typeof testimonialSchema>) {
    ...
    await activityService.log("create_testimonial", "testimonial", t.id, { client_name: data.client_name });
    return t;
  },

  async update(id: number, data: z.infer<typeof testimonialSchema>) {
    const t = await prisma.testimonial.update({ where: { id }, data });
    await activityService.log("update_testimonial", "testimonial", id, { client_name: data.client_name });
    return t;
  },

  async remove(id: number) {
    const t = await prisma.testimonial.findUnique({ where: { id } });
    if (!t) throw new AppError(404, "Témoignage introuvable");
    await prisma.testimonial.delete({ where: { id } });
    await activityService.log("delete_testimonial", "testimonial", id, { client_name: t.client_name });
  },
};

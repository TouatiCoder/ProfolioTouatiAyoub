...
export const serviceService = {
  async findAll() {
    return prisma.service.findMany({ orderBy: [{ sort_order: "asc" }, { id: "asc" }] });
  },

  async create(data: z.infer<typeof serviceSchema>) {
    const service = await prisma.service.create({ data });
    await activityService.log("create_service", "service", service.id, { name: data.name });
    return service;
  },

  async update(id: number, data: z.infer<typeof serviceSchema>) {
    const service = await prisma.service.update({ where: { id }, data });
    await activityService.log("update_service", "service", id, { name: data.name });
    return service;
  },
  ...
  async remove(id: number) {
    ...
    await activityService.log("delete_service", "service", id, { name: service.name });
  },
};
